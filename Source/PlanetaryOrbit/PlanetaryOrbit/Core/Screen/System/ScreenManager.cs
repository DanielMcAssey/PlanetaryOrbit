...
namespace PlanetaryOrbit.Core.Screen
{
    public class ScreenManager : DrawableGameComponent
    {
        ...
        GraphicsDeviceManager _obj_graphics = null;
        ContentManager _obj_content = null;
        InputManager _obj_input = null;
        ConfigManager _obj_config = null;
        ...
        Texture2D _screen_texture = null;
        float _screen_brightness = 255.0f;
        float _screen_contrast = 255.0f;
        BlendState _brightness_blend = null;
        BlendState _contrast_blend = null;

        public ScreenManager(Game _game, ConfigManager _config, GraphicsDeviceManager _graphicsmanager)
            : base(_game)
        {
            this._obj_config = _config;
            this._obj_graphics = _graphicsmanager;
        }
        ...
        /// <summary>
        /// Get Access to the Graphics Manager.
        /// </summary>
        ...
        {
            Logger.log(Log_Type.INFO, "Loading Screen Manager Content");
            try
            {
                // Load content belonging to the screen manager.
                this._obj_content = Game.Content;
                this._obj_input = new InputManager(ref this._obj_config);

                //Initialize non-default keyboard input.
                this._obj_input.AddKeyboardInput("GAME_START", Keys.Enter, true);
                this._obj_input.AddKeyboardInput("GAME_PAUSE", Keys.Escape, true);

                //Keyboard Only
                this._obj_input.AddKeyboardInput("SYSTEM_TOGGLE_FULLSCREEN", Keys.F12, true);

                this._obj_input.AddKeyboardInput("CAMERA_FORWARD", Keys.W, false);
                this._obj_input.AddKeyboardInput("CAMERA_BACK", Keys.S, false);
                this._obj_input.AddKeyboardInput("CAMERA_LEFT", Keys.A, false);
                this._obj_input.AddKeyboardInput("CAMERA_RIGHT", Keys.D, false);
                this._obj_input.AddKeyboardInput("CAMERA_UP", Keys.E, false);
                this._obj_input.AddKeyboardInput("CAMERA_DOWN", Keys.Q, false);
                this._obj_input.AddKeyboardInput("CAMERA_RESET", Keys.Space, true);
                this._obj_input.AddKeyboardInput("CAMERA_SPEED_TOGGLE", Keys.LeftShift, true);

                this._obj_input.AddKeyboardInput("PLANET_INFO", Keys.F1, true);
                this._obj_input.AddKeyboardInput("CAMERA_CHASE", Keys.F2, true);
                this._obj_input.AddKeyboardInput("CAMERA_ORBIT", Keys.F3, true);
                this._obj_input.AddKeyboardInput("CAMERA_TOP", Keys.F4, true);

                this._obj_input.AddKeyboardInput("CAMERA_ZOOM_IN", Keys.Add, false);
                this._obj_input.AddKeyboardInput("CAMERA_ZOOM_OUT", Keys.Subtract, false);
                this._obj_input.AddKeyboardInput("CAMERA_ZOOM_IN", Keys.X, false);
                this._obj_input.AddKeyboardInput("CAMERA_ZOOM_OUT", Keys.Z, false);  

                this._obj_input.AddKeyboardInput("PLANET_CHASE_NEXT", Keys.Right, true);
                this._obj_input.AddKeyboardInput("PLANET_CHASE_PREV", Keys.Left, true);

                this._obj_input.AddKeyboardInput("SIM_PAUSE", Keys.F6, true);
                this._obj_input.AddKeyboardInput("SIM_SPEED_DECREASE", Keys.F7, true);
                this._obj_input.AddKeyboardInput("SIM_SPEED_INCREASE", Keys.F8, true);
            }
            catch (Exception ex)
            {
            ...
            Logger.log(Log_Type.INFO, "Loading Global Spritebatch");
            try
            {
                
                this._obj_sprite_batch = new SpriteBatch(GraphicsDevice);
                this._obj_default_guifont = this._obj_content.Load<SpriteFont>(GameSettings.ASSET_CONFIG_GUI_FONT);
                this._obj_default_gamefont = this._obj_content.Load<SpriteFont>(GameSettings.ASSET_CONFIG_HUD_FONT);
                this._obj_default_debugfont = this._obj_content.Load<SpriteFont>(GameSettings.ASSET_CONFIG_DEBUG_FONT);
                this._obj_tex_blank = this._obj_content.Load<Texture2D>(GameSettings.ASSET_CONFIG_BLANK_BG);
            ...
            Logger.log(Log_Type.INFO, "Loading Global Spritebatch [Complete]");

            // Tell each of the screens to load their content.
            foreach (BaseScreen tscreen in this._list_screens_master)
            {
                Logger.log(Log_Type.INFO, "Loading Screen " + tscreen.ScreenName);
                try
                {
                    tscreen.loadContent();
                }
                ...
                Logger.log(Log_Type.INFO, "Loading Screen " + tscreen.ScreenName + " [Complete]");
            }

            _screen_texture = new Texture2D(GraphicsDevice, 1, 1);
            _screen_texture.SetData<Color>(new Color[] { Color.White });
            _screen_brightness = ((int)this.ConfigManager.CoreSettings.VIDEO_BRIGHTNESS) * _screen_brightness;
            _screen_contrast = ((int)this.ConfigManager.CoreSettings.VIDEO_CONTRAST) * _screen_contrast;
            _brightness_blend = new BlendState();
            _brightness_blend.ColorSourceBlend = _brightness_blend.AlphaSourceBlend = Blend.Zero;
            _brightness_blend.ColorDestinationBlend = _brightness_blend.AlphaDestinationBlend = Blend.SourceColor;

            _contrast_blend = new BlendState();
            _contrast_blend.ColorSourceBlend = _contrast_blend.AlphaSourceBlend = Blend.DestinationColor;
            _contrast_blend.ColorDestinationBlend = _contrast_blend.AlphaDestinationBlend = Blend.SourceColor;

            Logger.log(Log_Type.INFO, "Loading Screen Manager Content [Complete]");
        }
        ...
        public override void Draw(GameTime mGameTime)
        {
            ...
            SpriteBatch.Begin(SpriteSortMode.Immediate, _brightness_blend);
            SpriteBatch.Draw(_screen_texture, new Rectangle(0, 0, GameViewport.Width, GameViewport.Height), new Color(_screen_brightness, _screen_brightness, _screen_brightness, 255));
            SpriteBatch.End();

            SpriteBatch.Begin(SpriteSortMode.Immediate, _contrast_blend);
            SpriteBatch.Draw(_screen_texture, new Rectangle(0, 0, GameViewport.Width, GameViewport.Height), new Color(_screen_contrast, _screen_contrast, _screen_contrast, 255));
            SpriteBatch.End();
